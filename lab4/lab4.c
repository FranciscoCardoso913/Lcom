...
extern uint8_t packetByte, timer_counter;
...
int (mouse_test_packet)(uint32_t counter) {
  int ipc_status, r;
  uint8_t irq_set, counter_mouse = 0;
  message msg;
  struct packet pp;
      ...
      switch (_ENDPOINT_P(msg.m_source)) {
        case HARDWARE:
          if (msg.m_notify.interrupts & irq_set) {
            ...
            if(counter_mouse < 3) pp.bytes[counter_mouse++] = packetByte;

            if(counter_mouse == 3){
              mouse_parse_packet(&pp);
              mouse_print_packet(&pp);
              counter--, counter_mouse = 0;
            }
          }
          break;
        default: break;
...
int (mouse_test_async)(uint8_t idle_time) {
  int ipc_status, r;
  uint8_t irq_set_mouse, irq_set_timer, counter_mouse = 0;
  message msg;
  struct packet pp;
